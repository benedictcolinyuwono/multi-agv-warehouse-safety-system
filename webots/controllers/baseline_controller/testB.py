...
# Wider rule thresholds for bottom-corridor yielding.
BOTTOM_CORRIDOR_YIELD_WAIT_THRESHOLD = 1.8
AISLE_EXIT_REQUEST_THRESHOLD = 3.2
BOTTOM_TURN_CONFLICT_THRESHOLD = 2.2
...
def other_agv_near_aisle_exit_or_bottom_turn(
    aisle_name,
    exit_threshold=AISLE_EXIT_REQUEST_THRESHOLD,
    bottom_turn_threshold=BOTTOM_TURN_CONFLICT_THRESHOLD
):
    aisle_points = warehouse_waypoints[aisle_name]
    ...
    for other_name, ox, oy in get_other_agv_positions():
        # Once the AGV has reached the bottom wait point, it has joined the
        # bottom corridor queue, so normal bottom-slot protection handles it.
        if is_other_agv_at_bottom_wait_of_aisle(ox, oy, aisle_name):
            continue

        near_aisle_exit = math.hypot(ox - exit_x, oy - exit_y) < exit_threshold
        near_bottom_turn = math.hypot(ox - bottom_turn_x, oy - bottom_turn_y) < bottom_turn_threshold

        if near_aisle_exit or near_bottom_turn:
            return True, other_name
...
logger = ExperimentLogger(robot_name, "LEVEL_B")
...
while robot.step(timestep) != -1:
    ...
    # Shared aisle 1 / home staging queue control.
    ...
    # Bottom corridor slot protection.
    # This applies during both outbound and return movement.
    ...
    # Task aisle occupancy protection.
        ...
        if target_matches_point(target_x, target_y, first_x, first_y):
            ...
            if aisle_occupied or bottom_turn_occupied:
                record_wait_event(
                    ...
                    dt,
                    "AISLE_BLOCK",
                    f"aisle={task_aisle}, aisle_blocker={aisle_blocker}, bottom_turn_blocker={bottom_turn_blocker}"
                )
                stop_motors()
                logger.maybe_write_summary(current_time)
                continue

    # Top queue protection.
    if mode == GO_TO_TARGET:
        ...
        if is_top_queue:
            ...
            if occupied:
                record_wait_event(
                    ...
                    dt,
                    "AISLE_BLOCK",
                    f"top_queue={queue_aisle}, blocked_by={blocking_agv}"
                )
                stop_motors()
                logger.maybe_write_summary(current_time)
                continue

    # Return-home bottom corridor rules.
    if mode == RETURN_HOME:
        return_aisle = get_active_return_aisle()

        # 1) Protect the bottom wait point when the AGV is exiting its own task aisle.
        if return_aisle is not None and return_aisle.startswith("row_a_aisle_"):
            ...
            near_bottom_turn = is_near_point(
                current_x,
                current_y,
                bottom_turn_x,
                bottom_turn_y,
            ...
            if near_bottom_turn and targeting_bottom_wait:
                ...
                if occupied:
                    record_wait_event(
                        ...
                        dt,
                        "MERGE_YIELD",
                        f"bottom_turn_to_wait={return_aisle}, blocked_by={blocking_agv}"
                    )
                    stop_motors()
                    logger.maybe_write_summary(current_time)
                    continue

        # 2) Detect the AGV's actual current bottom corridor slot.
        ...
        if current_bottom_slot_aisle is not None:
            # 2a) Aisle-exit / bottom-turn priority rule.
            # If an AGV is near the exit OR in the bottom-turn merge area
            # of the next aisle to the left, hold this bottom-corridor AGV.
            next_left_aisle = get_next_left_aisle_name(current_bottom_slot_aisle)

            if next_left_aisle is not None:
                exit_conflict, exiting_agv = other_agv_near_aisle_exit_or_bottom_turn(next_left_aisle)
                ...
                if exit_conflict:
                    record_wait_event(
                        ...
                        f"yielding_to_exit_or_bottom_turn_agv={exiting_agv}, "
            ...
            # 2b) Existing bottom corridor next-slot protection.
